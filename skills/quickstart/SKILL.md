...
You are scaffolding a complete Even Hub G2 smart glasses project. Follow every step below in order. Do not skip any step. Use the project name derived from the user's request below (default to `my-evenhub-app` if none is given).
...
Extract the project name from the user's request. Strip spaces and special characters to produce a valid directory name (e.g. `"My Cool App"` → `my-cool-app`). If no argument is provided, use `my-evenhub-app`.
...
Scaffold a new Even Hub G2 project for the user's request below.
