...
If the user's request points to a file, read it directly. If it points to a directory, glob for `*.ts` and `*.tsx` files and read the most relevant ones (typically `main.ts`, `index.ts`, or the file containing `waitForEvenAppBridge`).
...
Analyze the plugin code at the path given in the user's request below. Identify all snapshot-worthy state variables following the table above. Then insert `setBackgroundState` + `onBackgroundRestore` at the correct location. Report a summary of:
